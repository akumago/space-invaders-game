...
export const processPlayerAction = (
  ...
  if (actionType === 'attack') {
    ...
    for (let i = 0; i < numberOfHits; i++) {
        ...
        let damage = criticalHit ? Math.floor(rawDamage * 1.5) : rawDamage;
      ...
      case SkillType.MAGIC:
        if (skill.target === TargetType.SINGLE_ENEMY) {
          ...
          if (skill.type === SkillType.MAGIC) {
            damage = baseSkillDamage + Math.floor(playerEffectiveStats.attack / 4); 
          } else { 
            damage = (baseSkillDamage * playerEffectiveStats.attack) - (enemyEffectiveStats.defense || 0);
          }
          ...
          if ( (skill.type === SkillType.ATTACK && (baseSkillDamage * playerEffectiveStats.attack) <= (enemyEffectiveStats.defense || 0) && damageMultiplier <= 1) || 
               (skill.type === SkillType.MAGIC && (baseSkillDamage + Math.floor(playerEffectiveStats.attack / 4)) <= 0 && damageMultiplier <= 1 )
            ) {
              damage = 0; 
          }
    ...
    switch (skillToUse.type) {
      ...
      case SkillType.MAGIC:
        ...
        if (skillToUse.type === SkillType.MAGIC) {
            damage = baseSkillDamage + Math.floor(enemyEffectiveStats.attack / 3); 
        } else { 
            damage = (baseSkillDamage * enemyEffectiveStats.attack) - playerEffectiveStats.defense;
        }
        ...
        damage = Math.max(1, Math.floor(damage));
         ...
         if ( (skillToUse.type === SkillType.ATTACK && (baseSkillDamage * enemyEffectiveStats.attack) <= (playerEffectiveStats.defense || 0) && damageMultiplier <= 1) ||
               (skillToUse.type === SkillType.MAGIC && (baseSkillDamage + Math.floor(enemyEffectiveStats.attack / 3)) <= 0 && damageMultiplier <= 1 )
            ) {
              damage = 0; 
         }
      ...
      default: 
         ...
         let dmgDefault = Math.max(1, rawDmgDefault);
         if (rawDmgDefault <= 0) dmgDefault = 0;
