...
import { PreTitleTapToStartScreen } from './components/PreTitleTapToStartScreen'; // Added
...
const App: React.FC = () => {
  const [gamePhase, setGamePhaseInternal] = useState<GamePhase>(GamePhase.PRE_TITLE_TAP_TO_START); // Initial phase
  ...
  const [isTitleScreenAudioActive, setIsTitleScreenAudioActive] = useState(false);

  const titleBgmAudioRef = useRef<HTMLAudioElement | null>(null); 
  ...
  const startNewGameFlow = () => {
    setIsTitleScreenAudioActive(false); 
    setPlayer(null); 
  ...
  const continueGameFlow = () => {
    // isTitleScreenAudioActive is already true if we reach here via PreTitleTapToStartScreen
    if (player) {
  ...
  const handleStopTitleMusic = useCallback(() => {
    if (titleBgmAudioRef.current) {
      titleBgmAudioRef.current.pause();
      titleBgmAudioRef.current.src = ''; 
      titleBgmAudioRef.current = null;
    }
  }, []);
  ...
  const setGamePhase = useCallback((phase: GamePhase, associatedData?: string) => {
    if (phase !== GamePhase.TITLE && phase !== GamePhase.PRE_TITLE_TAP_TO_START) {
  ...
  }, [regions, handleStopBattleMusic, handleStopEndingMusic, handleStopTitleMusic]); 

  useEffect(() => {
    if (gamePhase === GamePhase.TITLE && isTitleScreenAudioActive) {
        if (!titleBgmAudioRef.current) {
            titleBgmAudioRef.current = new Audio(BGM_FILES.TITLE);
            titleBgmAudioRef.current.crossOrigin = "anonymous";
            titleBgmAudioRef.current.loop = true;
            titleBgmAudioRef.current.volume = getMasterBgmVolume();
        }
        if (titleBgmAudioRef.current.paused) {
            titleBgmAudioRef.current.play().catch(e => console.warn("Title BGM play error:", e));
        }
    } else if (gamePhase !== GamePhase.PRE_TITLE_TAP_TO_START) { 
        handleStopTitleMusic();
    }
  }, [gamePhase, isTitleScreenAudioActive, handleStopTitleMusic]);
  ...
  useEffect(() => {
    if (gamePhase === GamePhase.BATTLE && currentRun && player) { 
      ...
      const isBoss = enemies.length === 1 && (
        ...
        enemies[0].id === ALL_ENEMIES.e_micchy_dragonlord.id || 
  ...
  const initiateRegionRun = (regionId: string) => {
    ...
    setCurrentRun({
      ...
      xpGainedThisRun: 0,
      goldGainedThisRun: 0,
      currentStageInBossFight: (regionId === REGIONS.r_micchy_tower.id && selectedRegion.encounters.length === 0) ? 1 : undefined, 
    });
  ...
  const handlePasswordLoadSuccess = (loadedPlayer: Player, loadedRegions: Record<string, Region>) => {
    ...
    setGamePhase(GamePhase.WORLD_MAP);
    setIsTitleScreenAudioActive(true); 
  };
  ...
  const handleProceedToTitleScreen = () => {
    handleAudioEnable(); // Ensure SFX system is initialized
    setIsTitleScreenAudioActive(true); // Ensure BGM will play and TitleScreen shows main menu
  ...
  const handleCreditsEnd = () => {
    ...
    setGamePhase(GamePhase.PRE_TITLE_TAP_TO_START); // Go back to tap-to-start
    setIsTitleScreenAudioActive(false); 
  };
  ...
  const renderScreen = () => {
    switch (gamePhase) {
      case GamePhase.PRE_TITLE_TAP_TO_START:
        return <PreTitleTapToStartScreen onProceed={handleProceedToTitleScreen} />;
      case GamePhase.TITLE:
        return <TitleScreen 
                  ...
                  isActive={gamePhase === GamePhase.TITLE} // Pass isActive prop
                />;
      ...
      case GamePhase.WORLD_MAP:
        if (!player) return <PreTitleTapToStartScreen onProceed={handleProceedToTitleScreen} />; // Fallback if player is null
      ...
      case GamePhase.GAME_OVER:
        if (!currentRun) { 
             ...
             setGamePhase(GamePhase.PRE_TITLE_TAP_TO_START); 
             return null;
        }
      ...
      case GamePhase.STATUS_SCREEN:
        if (!player) return <PreTitleTapToStartScreen onProceed={handleProceedToTitleScreen} />;
      ...
      default:
        // Fallback to PreTitleTapToStartScreen for unknown phases or if player data is missing when expected
        console.warn(`Unhandled game phase: ${gamePhase}, or missing player data. Resetting to PreTitle.`);
        setGamePhaseInternal(GamePhase.PRE_TITLE_TAP_TO_START);
        setIsTitleScreenAudioActive(false);
        return <PreTitleTapToStartScreen onProceed={handleProceedToTitleScreen} />;
    }
  };
