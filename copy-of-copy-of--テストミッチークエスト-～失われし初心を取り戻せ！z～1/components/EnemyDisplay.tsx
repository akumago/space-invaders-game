...
interface EnemyDisplayProps {
  ...
  isSfxEnabled: boolean; // New prop
}

export const EnemyDisplay: React.FC<EnemyDisplayProps> = ({ enemy, isSelected, onSelect, isLargeView = false, isFinalBoss = false, isLargeRegionalBoss = false, isSfxEnabled }) => {
  ...
  useEffect(() => {
    if (videoRef.current && enemy.isVideoSprite) {
      if (isFinalBoss) {
        ...
        videoRef.current.muted = !isSfxEnabled; // Unmute if SFX are globally enabled
      } else {
        videoRef.current.muted = true; // Other videos are always muted if autoplaying
      }
    }
  }, [enemy.isVideoSprite, enemy.spriteUrl, isFinalBoss, isSfxEnabled]);
      ...
      <div className="w-full h-full flex flex-col items-center justify-center text-center text-white animate-enemyAppear">
        <div className="flex-grow min-h-0 w-full flex items-center justify-center overflow-hidden p-1">
            ...
            <video
              ...
              autoPlay
              loop
              muted // Non-final boss videos in battle are always muted
              playsInline
      ...
      {enemy.isGeneratingSprite ? (
        ...
        <>
          <video
            ...
            autoPlay
            loop
            muted // Small view videos always muted
            playsInline
